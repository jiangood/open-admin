...
open-admin 设计为可嵌入框架，业务项目通过以下方式使用：

```
┌─────────────────────────────────────┐
│         你的业务项目                  │
│                                     │
│  pom.xml 添加 open-admin 依赖       │
│  package.json 添加 @jiangood/open-admin │
│                                     │
│  ┌─────────────────────────────┐    │
│  │      open-admin (嵌入)       │    │
│  │  用户/角色/菜单/字典/文件    │    │
...
│  ┌─────────────────────────────┐    │
│  │     你的业务代码             │    │
│  │  业务 Entity / Repository   │    │
│  │  业务页面 / 自定义组件       │    │
│  └─────────────────────────────┘    │
└─────────────────────────────────────┘
```

- **后端**：Maven 引入后自动注册配置（Security、JPA、Quartz），只需配置数据源即可
- **前端**：npm 引入后可直接使用框架组件库和全套管理页面
