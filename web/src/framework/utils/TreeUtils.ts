import { ArrUtil } from "./arr"; // 假设 ArrUtil 已经被正确定义和导出
...
export class TreeUtils {
     ...
     * @returns 包含层级信息的列表
     */
    private static _treeToList<T extends TreeNode>(tree: T[], level: number, result: (T & { level: number })[]): (T & { level: number })[] {
...
/**
 * 📜 根据键值列表查找所有匹配的节点列表。
 * 注意：此方法依赖于一个名为 ArrUtil 的外部工具类的 `contains` 方法。
