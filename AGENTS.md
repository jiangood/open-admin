...
- `docs/open-admin/` 与 `.opencode/skills/oa-crud`、`oa-upgrade` 会被打包进 JAR，并在**业务项目启动后端时自动同步**到其根目录（`FrameworkFileSyncer` 按内容比对）。修改这些文件 = 修改框架对外 API，必须 `mvn clean install` 后在示例项目验证。`oa-publishing-release` skill 仅本仓库使用，不打进 JAR
