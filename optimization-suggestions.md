...
> 本文档包含 31 条优化建议，涵盖后端、前端、构建配置、安全性、性能、代码质量、测试、架构设计等方面。
>
...
### P3 XSS 风险：`dangerouslySetInnerHTML` 未做清理 🔴 ⭐⭐
...
### P24 前端代码中使用 `// eslint-disable-next-line` 🟢 ⭐
...
### P27 `renderCenterContent` 每次创建新函数 🟢 ⭐
...
|------|------|------|
| D1 | 缺少 Dockerfile | 构建 |
| D2 | Maven `revision` 占位符在 IDE 中不友好 | 构建 |
| D3 | `@umijs/types` 版本与 UmiJS 版本不匹配 | 构建 |
| D4 | JavaDoc 缺失严重 | 文档 |
| D5 | 缺少架构决策记录 (ADR) | 文档 |
| D6 | 变更日志不规范 | 文档 |
| D7 | CONTRIBUTING.md 缺失 | 文档 |
| D8 | `TabPageRender` 可能无限增长 | 前端 |
| D9 | 登录页面加载状态永久卡死 | 前端 |
| D10 | 测试覆盖率低（Spec、SysUserService、PermissionAspect） | 测试 |